...
// Keep the HGS/OGS toll debt in step with the contract's HGS check block: a
// confirmed, reflected debt is carried as an auto return-charge line (category
// HGS_OGS, source HGS) so it rolls into returnExtraAmount / the grand total and
// shows up on the Dönüş Ekstra tab. Anything else clears our line. Always
// recomputes the cached financials afterwards.
const syncHgsCharge = async (contractId, client) => {
  const contract = await client.contract.findUnique({ where: { id: contractId } });
  await client.contractReturnCharge.deleteMany({ where: { contractId, source: "HGS" } });

  const amount = round2(num(contract.hgsAmount) || 0);
  if (contract.hgsStatus === "DEBT" && contract.hgsReflected && amount > 0) {
    await client.contractReturnCharge.create({
      data: {
        contractId,
        category: "HGS_OGS",
        description: `HGS / OGS kontrolü · ${trDate(contract.hgsCheckedFrom)} – ${trDate(contract.hgsCheckedTo)}`,
        amount,
        quantity: 1,
        autoGenerated: true,
        source: "HGS",
  ...
  return recomputeContractFinancials(contractId, client);
};
...
const updateContract = asyncHandler(async (req, res) => {
  ...
  const contract = await prisma.$transaction(async (tx) => {
    ...
    // Mirror the HGS toll debt into a return-charge line, then recompute
    // returnExtraAmount / totalPrice / the ledger debit from the real rows.
    await syncHgsCharge(existing.id, tx);
    return tx.contract.findUnique({ where: { id: existing.id }, include: CAR_INCLUDE });
  });
