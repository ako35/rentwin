...
const setContractStatus = (status) =>
  asyncHandler(async (req, res) => {
    ...
    const contract = await prisma.contract.update({
      ...
      data: {
        status,
        // Reopening clears the recorded hand-back; the auto return charges from
        // that return stay (operator removes them via the Dönüş Ekstrası tab).
        ...(status === "CREATED"
          ? { returnKm: null, returnFuelEighths: null, returnedAt: null }
          : {}),
      },
    });
...
const returnContract = asyncHandler(async (req, res) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // Replace only this flow's own rows — manual return charges are left alone.
    await tx.contractReturnCharge.deleteMany({
      where: { contractId: existing.id, autoGenerated: true },
    });
    if (charges.length) {
